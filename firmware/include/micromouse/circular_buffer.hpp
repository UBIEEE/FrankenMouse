// Adapted from
// https://github.com/wpilibsuite/allwpilib/blob/7ca35e5678cf32caec6a1a866ca51d0136c4c398/wpiutil/src/main/native/include/wpi/static_circular_buffer.h
//
 ...
 public:
  ...
  class iterator {
   public:
    ...
    constexpr iterator(CircularBuffer* buffer, size_t index) : m_buffer{buffer}, m_index{index} {}
  ...
  class const_iterator {
   public:
    ...
    constexpr const_iterator(const CircularBuffer* buffer, size_t index) : m_buffer{buffer}, m_index{index} {}
   ...
   */
  constexpr iterator end() { return iterator(this, CircularBuffer<T, N>::size()); }
   ...
   */
  constexpr const_iterator end() const { return const_iterator(this, CircularBuffer<T, N>::size()); }
   ...
   * Returns const end iterator.
   */
  constexpr const_iterator cend() const { return const_iterator(this, CircularBuffer<T, N>::size()); }
   ...
   * If there are no elements in the buffer, calling this function results in undefined behavior.
   */
  constexpr const T& back() const { return m_data[(m_front + m_length - 1) % N]; }
  ...
  /**
   * Push a new value onto the front of the buffer that is constructed with the provided constructor
   * arguments.
   *
   * The value at the back is overwritten if the buffer is full.
   */
   ...
   */
  constexpr T& operator[](size_t index) { return m_data[(m_front + index) % N]; }

  /**
   * @return Element at index starting from front of buffer.
   */
  constexpr const T& operator[](size_t index) const { return m_data[(m_front + index) % N]; }
