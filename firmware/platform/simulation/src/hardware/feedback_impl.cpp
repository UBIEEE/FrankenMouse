...
FeedbackImpl::FeedbackImpl() : Node("micromouse_feedback") {
  m_main_task_sub = this->create_subscription<std_msgs::msg::UInt8MultiArray>(
      "/client/main/task", 10, [this](const std_msgs::msg::UInt8MultiArray& msg) {
        Robot::get().delegate_received_feedback(feedback::TopicReceive::MAIN_TASK, msg.data.data());
      });

  m_main_command_sub = this->create_subscription<std_msgs::msg::UInt8>(
      "/client/main/command", 10, [this](const std_msgs::msg::UInt8& msg) {
        Robot::get().delegate_received_feedback(feedback::TopicReceive::MAIN_COMMAND, &msg.data);
      });

  m_main_song_sub = this->create_subscription<std_msgs::msg::UInt8>(
      "/client/main/song", 10, [this](const std_msgs::msg::UInt8& msg) {
        Robot::get().delegate_received_feedback(feedback::TopicReceive::MAIN_SONG, &msg.data);
      });

  m_drive_pid_sub = this->create_subscription<std_msgs::msg::Float32MultiArray>(
      "/client/drive/pid", 10, [this](const std_msgs::msg::Float32MultiArray& msg) {
        (void)msg;
  ...
  m_vision_distances_pub =
      this->create_publisher<std_msgs::msg::Float32MultiArray>("/robot/vision/distances", 10);
  m_drive_motor_data_pub =
      this->create_publisher<std_msgs::msg::Float32MultiArray>("/robot/drive/motors", 10);
  m_drive_imu_data_pub = this->create_publisher<std_msgs::msg::Float32MultiArray>("/robot/drive/imu", 10);
  m_drive_pid_data_pub = this->create_publisher<std_msgs::msg::Float32MultiArray>("/robot/drive/pid", 10);
  m_drive_chassis_speeds_pub =
      this->create_publisher<geometry_msgs::msg::Twist>("/robot/drive/chassis_speeds", 10);
  m_maze_cell_pub = this->create_publisher<std_msgs::msg::UInt8MultiArray>("/robot/maze/cell", 10);
  m_maze_coordinates_pub = this->create_publisher<std_msgs::msg::UInt8>("/robot/maze/coordinates", 10);
}
