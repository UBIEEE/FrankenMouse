...
static const SimpleBLE::BluetoothUUID s_characteristics[_CHAR_COUNT] = {
    // Main (0000-0009)
    [CHAR_MAIN_TASK] = "00000000-8e22-4541-9d4c-21edae82ed19"s,
    [CHAR_MAIN_COMMAND] = "00000001-8e22-4541-9d4c-21edae82ed19"s,
    [CHAR_MAIN_ERROR] = "00000002-8e22-4541-9d4c-21edae82ed19"s,
    [CHAR_MAIN_SONG] = "00000003-8e22-4541-9d4c-21edae82ed19"s,
    [CHAR_MAIN_STATUS] = "00000004-8e22-4541-9d4c-21edae82ed19"s,

    // Vision (000A-000E)
    [CHAR_VISION_RAW_READINGS] = "0000000a-8e22-4541-9d4c-21edae82ed19"s,
    [CHAR_VISION_DISTANCES] = "0000000b-8e22-4541-9d4c-21edae82ed19"s,

    // Drive (000F-0013)
    [CHAR_DRIVE_MOTOR_DATA] = "0000000f-8e22-4541-9d4c-21edae82ed19"s,
    [CHAR_DRIVE_IMU_DATA] = "00000010-8e22-4541-9d4c-21edae82ed19"s,
    [CHAR_DRIVE_PID] = "00000011-8e22-4541-9d4c-21edae82ed19"s,
    [CHAR_DRIVE_CHASSIS_SPEEDS] = "00000012-8e22-4541-9d4c-21edae82ed19"s,

    // Maze (0014-0018)
    [CHAR_MAZE_CELL] = "00000014-8e22-4541-9d4c-21edae82ed19"s,
    [CHAR_MAZE_MOUSE_POSITION] = "00000015-8e22-4541-9d4c-21edae82ed19"s,
};
...
bool BLEManager::initialize_adapter() {
  ...
  if (m_adapter_index >= adapters.size()) {
    report_error(name(), "adapter index %zu out of range [0, %zu)",
...
void BLEManager::on_connected() {
  ...
  write<BLETopicWrite::MAIN_COMMAND>(RobotCommand::RESEND_ALL_FEEDBACK);
...
void BLEManager::configure_peripheral_notifications() {
  ...
  notify<BLETopicNotify::MAIN_ERROR>(
      [&](const RobotError& error) { m_main_data.errors[error.timestamp] = error; });
  notify<BLETopicNotify::MAIN_SONG>(m_main_data.song);
  notify<BLETopicNotify::MAIN_STATUS>(
      [&](const std::pair<RobotStatusTopic, uint8_t>& status) {
  ...
  notify<BLETopicNotify::DRIVE_CHASSIS_SPEEDS>(m_drive_data.chassis_speeds);

  notify<BLETopicNotify::MAZE_CELL>(
      [&](const std::pair<Coordinate, Cell>& cell_with_position) {
        const auto& [position, cell] = cell_with_position;
        m_maze_data.cells[position] = cell;
      });

  notify<BLETopicNotify::MAZE_MOUSE_POSITION>(m_maze_data.mouse_position);
}

BLEService BLEManager::get_characteristic_service(BLECharacteristic char_id) {
  switch (char_id) {
    case CHAR_MAIN_TASK:
    case CHAR_MAIN_COMMAND:
    case CHAR_MAIN_ERROR:
    case CHAR_MAIN_SONG:
    case CHAR_MAIN_STATUS:
      return SERVICE_MAIN;
    ...
    case CHAR_DRIVE_PID:
    case CHAR_DRIVE_CHASSIS_SPEEDS:
      return SERVICE_DRIVE;
    case CHAR_MAZE_CELL:
    case CHAR_MAZE_MOUSE_POSITION:
      return SERVICE_MAZE;
    default:
      assert(false);// TODO
