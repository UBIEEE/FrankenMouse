...
class ArgumentParser final {
 ...
 public:
  ArgumentParser(std::span<const std::string> args, std::span<const Option> options)
  ...
  const std::unordered_set<int>& parsed_options() const { return m_parsed_options; }
  const std::unordered_map<int, std::string_view>& parsed_option_values() const {
    return m_parsed_option_values;
  }

  std::span<const std::string_view> non_option_args() const { return m_non_option_args; }
