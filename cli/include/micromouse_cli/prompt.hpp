...
class Prompt final {
 ...
 public:
  ...
  /**
   * Adds a command to the list of available input commands for this prompt.
   *
   * @tparam T Type of the command to register. Must conform to the CommandType concept.
   */
  ...
  void register_command() {
    ...
    register_command(name, std::move(prompt_info), std::move(make_command_func));
  }

  void register_command(const char* name, Command::PromptInfo prompt_info, MakeCommandFunc make_command);
  ...
  /**
   * Prompts the user for input. The input is parsed and the corresponding command is invoked.
   *
   ...
   *
   * @param command On success, this will be set to a newly allocated instance of the invoked command. The
   *                caller is responsible for freeing this instance.
   *
 ...
 private:
  ...
  void complete_command_name(struct ic_completion_env_s* cenv, const char* word);
  ...
  /**
   * Parses the first word of the input string, matches it against the registered commands, and returns the
   * corresponding command iterator if found.
   *
 ...
 private:
  /**
   * Return a pointer to the beginning of the next word in the input string. The length of the word is stored
   * in word_len.
   *
   * @param input     Pointer to the input string (cannot be nullptr). The pointer is updated to point to the
   *                  character following the word.
   * @param word_len  Length of the word found (cannot be nullptr).
   * @param is_quoted (Optional output) Set to true if the word was surrounded by quotes (single or double)
   *
   * @return A pointer to the beginning of the next word in the input string, or nullptr if no word is found.
   */
  static const char* get_next_word(const char** input, size_t* word_len, bool* is_quoted = nullptr);
  ...
  /**
   * Returns true if the cursor is positioned in the first word of the input string.
   *
